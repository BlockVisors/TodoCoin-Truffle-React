...
function testReward() {
    ...
    uint actualRecipientBalance = 
    taskMaster.getBalance(recipientAddress);
    Assert.equal(actualRecipientBalance, expectedRecipientBalance, 
    "Recipient should have 50 wei");

    uint expectedOwnerBalance = 9950;
    uint actualOwnerBalance = 
     taskMaster.getBalance(taskMaster.owner());
    Assert.equal(actualOwnerBalance, expectedOwnerBalance, "Owner 
    should have 9950 wei");
}